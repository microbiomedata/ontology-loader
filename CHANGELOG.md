...
- **NCBITaxon-scale ancestry closure computation took ~29 minutes for one closure type, dominated by one `adapter.ancestors()` SQL traversal per entity (~2.7M individual calls).** `get_relations_closure`'s ancestry phase now reads directly from semsql's own `entailed_edge` table, which already materializes the full transitive closure, instead of recomputing it one entity at a time. Measured against the real NCBITaxon semsql database and a real prior run's own log: 29m9s (the old per-entity loop) versus 13.0s for a bulk scan of all 51,991,442 rows — about 134x faster for that phase. Correctness verified against the old `adapter.ancestors()` output on real envo entities (exact match), and covered by a new regression test comparing the two approaches directly rather than mocking either one. Streamed via a generator rather than `fetchall()`, since NCBITaxon-scale results are tens of millions of rows. See https://github.com/microbiomedata/ontology-loader/issues/18.
